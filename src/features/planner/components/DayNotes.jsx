...
export const DayNotes = ({ content = '', onContentChange, isSaving, dateStr }) => {
  const maxLength = 300;
  ...
  return (
    <Card hoverEffect={false} className="flex flex-col gap-3">
      <SectionHeader
        icon={StickyNote}
        title="Planner Notes"
        subtitle={`Notes & thoughts for ${dateStr}`}
        rightAction={
      ...
      <div className="relative">
        <textarea
          ...
          placeholder="Capture today's thoughts, key outcomes, or notes..."
          className="w-full h-28 p-3.5 bg-slate-50 border border-[#E2E8F0] rounded-xl text-sm text-[#0F172A] placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white resize-none transition-all duration-150"
        />
